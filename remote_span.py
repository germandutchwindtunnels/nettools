...
def configure_remote_span(srcswitch, srcport, srcinterface, dstswitch, dstinterface, spanvlan, user, pwd):  # pylint: disable=too-many-arguments
    ...
    source.remote_span(span_session_number, "interface " +
                       srcinterface, "remote vlan " + str(spanvlan))

    print "Setting destination switch " + dstswitch
    dest = CiscoTelnetSession()
    dest.open(dstswitch, dstport, user, pwd)
    dest.remote_span(span_session_number, "remote vlan " +
                     str(spanvlan), "interface " + dstinterface)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 5:
        sys.stderr.write("Usage: " + sys.argv[0] +
                         " username password source-switch clear span-vlan-nr\n")
        sys.stderr.write("Usage: " + sys.argv[0] + " username password source-switch list\n")
        sys.stderr.write(
            "Usage: " +
            sys.argv[0] +
            " username password source-switch source-port destination-switch destination-port span-vlan-nr\n")
        sys.stderr.write(
            "Usage: " +
            sys.argv[0] +
            " username password source-switch source-port destination-switch destination-port\n")
        sys.stderr.write(
            "Usage: " +
            sys.argv[0] +
            " username password source-switch source-port destination-switch\n")
    ...
    if src_switch is not None and \
        ...
        # dst_switch is now span-vlan-nr
        discover_erase_span(username, password, src_switch, dst_switch)
        ...
        configure_remote_span(src_switch, port, src_port, dst_switch,
                              dst_port, span_vlan, username, password)
    else:
