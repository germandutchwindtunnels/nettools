...
from PyQt4.QtGui import QApplication, QPushButton, QComboBox, QTableWidgetItem, QColor, QCursor
from PyQt4.QtCore import QString, QVariant, QThread, Qt
...
class PortConfigGui(QtGui.QMainWindow):
	...
	def load_patchports(self):
		while self.patchports == None:
			QApplication.setOverrideCursor(QCursor(Qt.WaitCursor))
			self.patchports = portconfig.get_available_patchports(self.hostname, 23, self.username, self.password)
			QApplication.restoreOverrideCursor()

	def load_vlans(self):
		while self.vlans == None:
			self.vlans = portconfig.get_available_vlans(self.hostname, 23, self.username, self.password)

	def load_content_background(self):
		pass	
	...
	def config_row(self, row_number):
		...
		comboBoxPatchport.currentIndexChanged.connect(lambda i, row_number = row_number: self._patchport_selected(row_number))
		comboBoxVlans.currentIndexChanged.connect(lambda i, row_number = row_number: self._vlanname_selected(row_number))		
		buttonSubmit.clicked.connect(lambda i, row_number = row_number: self._submit_pressed(row_number))	
		...
		comboBoxVlans.currentIndexChanged.connect(lambda i, row_number = row_number: self._row_edited(row_number))		
		comboBoxPatchport.currentIndexChanged.connect(lambda i, row_number = row_number: self._row_edited(row_number))		
	...
	def _get_combobox_variantdata(self, row_number, column_number):
		...
		variantDict = eval(variantString)
		return variantDict
	...
	def _patchport_selected(self, row_number):
		...
		#Ports which are configured as dynamic may show up as "unassigned" when nothing is connected
	...
	def _submit_pressed(self, row_number):
		...
		QApplication.setOverrideCursor(QCursor(Qt.WaitCursor))
		portconfig.configure_patchid_raw(username, password, switch_hostname, switchport, vlanid, old_vlanid)
		QApplication.restoreOverrideCursor()
...
if __name__ == '__main__':
	...
	pcg = PortConfigGui(hostname, sys.argv[1], sys.argv[2])
	pcg.show()
	pcg.redirect_stdout()
	pcg.redirect_stderr()
